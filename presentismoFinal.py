...
class AplicacionPresentismo(QWidget):
    """
    Pestaña Presentismo completa:
    - carga múltiples excels (.xls/.xlsx) (lee todas las hojas si vienen en varios archivos)
    - detecta columna Nombre y fecha/hora (Marc. o Fecha/Hora)
    - separa Fecha y Hora
    - muestra solo: Nombre | Fecha/Hora | Fecha | Hora | Sector | Delay (si aplica)
    - asignación automática potente (fuzzy + token matching)
    - MatchScore/MatchedName guardados en df si se desea (no visible por defecto)
    - ver sin sector, asignación manual mejorada, asignar y guardar en sectores.json
    - editor visual de sectores y editor visual de horarios (sector / persona)
    - filtros: primer fichaje del día y retrasos (según horarios configurados)
    - resaltado amarillo para retrasos y columna con tiempo de retraso
    """
    ...
    def __init__(self):
        ...
        # DataFrames
        self.df = pd.DataFrame()          # Data procesada actual
        self.df_original = pd.DataFrame() # Raw concatenado
        self.display_df = pd.DataFrame()  # Lo que se muestra (posible subset)
    ...
    def _init_ui(self):
        ...
        self.tabla.setHorizontalHeaderLabels(["Nombre", "Fecha/Hora (raw)", "Fecha", "Hora", "Sector", "Retraso", "Horario Ref"])
        ...
        self.chk_primero = QCheckBox("Primer fichaje del día")
        ...
        btn_apply = QPushButton("✅ Aplicar filtros y recalcular")
        ...
        right_col.addSpacing(8)
        right_col.addWidget(QLabel("📅 Fechas a eliminar:"))
        ...
        # refresco automático de la tabla si hay cambios - útil para ver asignaciones en tiempo real
        self._auto_refresh_timer = QTimer()
        self._auto_refresh_timer.setInterval(800)  # ms
        self._auto_refresh_timer.timeout.connect(lambda: None)  # placeholder (no hace nada por ahora)
    ...
    # -------------------------
    # Leer múltiples Excel (varios archivos)
    # - intenta leer con pandas (openpyxl/xlrd según extensión)
    # - concatena todas las hojas/dataframes
    # - detecta columnas Nombre y Fecha/Hora (Marc.)
    # - parseo robusto de Fecha/Hora (varios formatos + dateutil)
    # - separa Fecha y Hora y mantiene string original en "FechaHora_raw"
    # -------------------------
    def cargar_varios_excels(self):
        archivos, _ = QFileDialog.getOpenFileNames(self, "Seleccionar archivos Excel", "", "Archivos Excel (*.xlsx *.xls *.csv)")
        if not archivos:
            return
        ...
        for archivo in archivos:
            try:
                ...
                if ext == ".xls":
                    # xlrd puede lanzar advertencias si el archivo es antiguo; aún así intentamos
        ...
        if errores:
            QMessageBox.warning(self, "Aviso lectura archivos", "Algunos archivos no se pudieron leer:\n" + "\n".join(errores))
        ...
        # detectar columna Fecha/Hora (Marc. o Fecha/Hora)
        col_fecha = None
        for c in cols:
            cl = str(c).lower()
        ...
        if col_fecha is None:
            for c in cols:
                ...
                if "marc" in cl or "marc." in cl or "marca" in cl or ("date" in cl and "time" in cl):
                    ...
                    break
        if col_fecha is None:
            # fallback: choose second column if likely
            if len(cols) >= 2:
                col_fecha = cols[1]
            else:
        ...
        # parseo robusto de fecha/hora: probamos formatos comunes y dateutil parser
        def parse_seguro(val_raw):
            ...
            if s == "" or s.lower() in ("nan", "none", "na", "n/a"):
                return None
            # limpiar caracteres no imprimibles y nbsp
            ...
            # probar formatos explícitos (con dayfirst=True para los dd/mm)
            for fmt in formatos:
                try:
                    # utilizar pandas for speed and consistent dtype
                    dt = pd.to_datetime(s, format=fmt, dayfirst=True, errors="raise")
                    return dt
                except Exception:
                    continue
            # intentar dateutil parser (fuzzy) con dayfirst True
            try:
                dt = parser.parse(s, dayfirst=True, fuzzy=True)
                return pd.to_datetime(dt)
            except Exception:
                # intentar sin dayfirst (fallback)
                try:
        ...
        # Mantener Fecha y Hora separadas; si no se pudo parsear, Fecha será NaT y Hora como texto (raw)
        df_min["Fecha"] = df_min["FechaHora_parsed"].dt.date
        df_min["Hora"] = df_min["FechaHora_parsed"].dt.time
        # para registros no parseados, almacenamos el raw en "Hora_text"
        df_min["Hora_text"] = df_min.apply(lambda r: "" if pd.notna(r["Hora"]) else r["FechaHora_raw"], axis=1)

        # inicializar columnas adicionales
        df_min["Sector"] = df_min["Nombre"].apply(self._auto_asign_sector_with_threshold)
        df_min["MatchedName"] = ""   # por auditoría si queremos
    ...
    # -------------------------
    # Asignación automática con umbral configurable
    # -------------------------
    ...
    def _asignar_sector(self, nombre, umbral=85, guardar_matchinfo=True):
        """
        Asigna sector usando fuzzy matching sobre la lista de sectores cargada.
        Retorna nombre de sector o "No asignado".
        Si guardar_matchinfo True, rellena MatchedName/MatchScore en self.df (si existe índice).
        """
        ...
        for sector, lista in self.sectores.items():
            for persona in lista:
                ...
                # combinar token_set, token_sort y partial
    ...
    # -------------------------
    # Mostrar tabla (df puede ser subset)
    # - colorea filas con retraso (amarillo)
    # - colorea filas con fecha/hora inválida (rojo suave)
    # -------------------------
    def _actualizar_tabla(self, df=None):
        ...
        for i, row in self.display_df.iterrows():
            ...
            if pd.notna(hora_val) and hora_val is not None:
                ...
                # fallback a Hora_text si existe
            ...
            # resaltar no asignados con color amarillo claro background for sector cell
    ...
    # -------------------------
    # Ventana visual para asignar sector (mejorada)
    # - muestra en la izquierda la lista de personas (desde df)
    # - derecha: lista de sectores; botón asignar (y agregar al sector)
    # -------------------------
    ...
    # -------------------------
    # Editor de sectores (mejor visual)
    # - izquierda: sectores
    # - derecha: miembros con add/remove
    # -------------------------
    def abrir_editor_sectores(self):
        ...
        def del_sector():
            ...
            if not cur:
                return
            sec = cur.text()
            if QMessageBox.question(dlg, "Confirmar", f"Eliminar sector '{sec}' y sus empleados?") == QMessageBox.StandardButton.Yes:
    ...
    # -------------------------
    # Editor de horarios (mejor visual) - MODIFICACIÓN: Ahora soporta horarios por día
    # - lista sectores; al seleccionar se muestran horarios del sector y personas
    # - permite definir horario por sector (__sector__) y por persona, PERO AHORA POR DÍA
    # -------------------------
    ...
    def abrir_editor_horarios(self):
        ...
        dlg.resize(1400, 800)  # Más ancho para overrides
        root = QHBoxLayout(dlg)
        ...
        # controls to add sector from here
        inp_new_sector = QLineEdit()
        inp_new_sector.setPlaceholderText("Nuevo sector...")
        left.addWidget(inp_new_sector)
        btn_add_sector = QPushButton("➕ Agregar sector")
        ...
        # right area: show sector-level schedule and per-person schedules
        right = QVBoxLayout()
        right.addWidget(QLabel("Horario del sector por día (HH:MM) — deja vacío si no aplica:"))

        # MODIFICACIÓN: Form para días del sector
        ...
        # MODIFICACIÓN: Form para días del empleado
        ...
        # NUEVO: Sección de overrides por fecha
        ...
        def refresh_overrides():
            ...
            # Asumir overrides en self.horarios["__overrides__"] = {fecha: {emp: time}}
        ...
        def refresh_emps_and_sector():
            ...
            if not cur:
                return
            sec = cur.text()
            # MODIFICACIÓN: Cargar horarios del sector por día
            if sec in self.horarios:
                ...
                if isinstance(sec_entry, dict) and "__sector__" in sec_entry:
                    ...
                    elif isinstance(sector_horas, str):  # Compatibilidad con datos antiguos
                        for dia in dias:
                            sector_inputs[dia].setText(sector_horas)  # Aplicar a todos los días
            # empleados
            miembros = sorted(self.sectores.get(sec, []))
            for m in miembros:
                lw_emps.addItem(m)
            # Actualizar combo de empleados para overrides
        ...
        def on_emp_click():
            ...
            if sec in self.horarios and emp in self.horarios[sec]:
                ...
                elif isinstance(emp_horas, str):  # Compatibilidad
                    for dia in dias:
        ...
        def add_override():
            ...
            time = self.override_time.text().strip()
            if not fecha or not emp or not time:
            ...
            self.horarios["__overrides__"][fecha][emp] = time
            refresh_overrides()
            QMessageBox.information(dlg, "Agregado", f"Override agregado para {emp} en {fecha}: {time}")
        ...
        def remove_override():
            ...
            if not cur:
                ...
                return
            text = cur.text()  # e.g. "2023-10-01 - Juan: 09:00"
            parts = text.split(" - ")
            if len(parts) != 2:
                return
            fecha = parts[0]
            ...
            emp = emp_time[0]
            time = emp_time[1]
            if "__overrides__" in self.horarios and fecha in self.horarios["__overrides__"] and emp in self.horarios["__overrides__"][fecha]:
    ...
    # -------------------------
    # Filtro: primer fichaje del dia (mantener primer registro por Nombre+Fecha)
    # -------------------------
    def _filter_primer_fichaje(self, df):
        if df.empty:
            return df
        # ordenar por Nombre y FechaHora_parsed (None al final)
        df2 = df.copy()
        # crear columna usable para ordenar: si FechaHora_parsed es NaT -> max
        df2["_sort_dt"] = df2["FechaHora_parsed"].apply(lambda x: pd.Timestamp.max if pd.isna(x) else x)
        df2 = df2.sort_values(["Nombre", "_sort_dt"], ascending=[True, True])
        # agrupar por Nombre+Fecha (si Fecha es NaT agrupamos por FechaHora_raw -> en ese caso no se eliminarán)
        # convertir Fecha a string para agrupar NaT de forma consistente
    ...
    # -------------------------
    # Filtro: detectar retrasos según horarios.json - MODIFICACIÓN: Ahora consulta por día de la semana
    # -------------------------
    def _filter_retrasos(self, df):
        ...
        df2 = df.copy()
        df2["Delay"] = pd.NaT
        df2["Horario_Referencia"] = ""  # Nueva columna para debugging

        dias_semana = ["lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo"]

        # helper to find reference time (datetime.time) for a row
        def get_ref_time(row):
            sec = row.get("Sector", "")
            nombre = row.get("Nombre", "")
            fecha_str = str(row.get("Fecha"))
            # Check for date-specific overrides first
            if "__overrides__" in self.horarios and fecha_str in self.horarios["__overrides__"] and nombre in self.horarios["__overrides__"][fecha_str]:
                return self.horarios["__overrides__"][fecha_str][nombre]

            # Obtener día de la semana (0=lunes, 6=domingo)
            if pd.notna(row.get("FechaHora_parsed")):
                dia_idx = row["FechaHora_parsed"].weekday()
                dia = dias_semana[dia_idx]
            else:
                return None  # Sin fecha, no podemos determinar día

            # Buscar horario por persona primero
            if sec in self.horarios:
                sec_entry = self.horarios[sec]
                if isinstance(sec_entry, dict):
                    ...
                    if nombre in sec_entry:
                        ...
                        elif isinstance(persona_horas, str):  # Compatibilidad
                            return persona_horas
                    # Luego sector default
                    if "__sector__" in sec_entry:
                        sector_horas = sec_entry["__sector__"]
                        if isinstance(sector_horas, dict) and dia in sector_horas:
                            return sector_horas[dia]
                        elif isinstance(sector_horas, str):  # Compatibilidad
                            return sector_horas
                elif isinstance(sec_entry, str):  # Compatibilidad total
                    return sec_entry
            return None

        for idx, row in df2.iterrows():
            ref = get_ref_time(row)
            df2.at[idx, "Horario_Referencia"] = ref if ref else ""  # Guardar referencia para debugging
            if not ref:
                df2.at[idx, "Delay"] = pd.NaT
                continue

            # obtener hora real (igual que antes)
            hora_real = None
            if pd.notna(row.get("FechaHora_parsed")):
            ...
            # convertir ref (string) a time (igual que antes)
            try:
    ...
    def _filter_eliminar_fechas(self, df):
        if df.empty or not self.fechas_a_eliminar:
            return df
        # Convertir fechas_a_eliminar a set para búsqueda rápida
        fechas_set = set(self.fechas_a_eliminar)
        # Filtrar filas donde str(Fecha) no esté en fechas_set
        return df[~df["Fecha"].astype(str).isin(fechas_set)]

    # -------------------------
    # Aplicar filtros seleccionados en la UI
    # -------------------------
    ...
    def exportar_vista_actual(self):
        if self.display_df.empty:
            ...
            return
        path, _ = QFileDialog.getSaveFileName(self, "Guardar vista actual (Excel)", "", "Archivos Excel (*.xlsx)")
        if not path:
            return
        try:
    ...
    def exportar_resultado(self):
        if self.display_df.empty:
            ...
            return
        # Ensure necessary filters are applied for presentismo calculation
        if "Delay" not in self.display_df.columns:
            self.display_df = self._filter_retrasos(self.display_df)
        # Always apply primer fichaje for presentismo (to avoid duplicates per day)
        self.display_df = self._filter_primer_fichaje(self.display_df)
        # Apply eliminar fechas if any
        self.display_df = self._filter_eliminar_fechas(self.display_df)

        path, _ = QFileDialog.getSaveFileName(self, "Guardar resultado (Excel)", "", "Archivos Excel (*.xlsx)")
        if not path:
            return
        try:
            # Determinar título basado en sector, mes y año de los datos
            fecha_max = self.display_df['Fecha'].max()
            if pd.notna(fecha_max):
                month = fecha_max.strftime('%B').upper()
                year = fecha_max.year
            else:
                month = 'OCTUBRE'
                year = 2025
            sectores_unicos = self.display_df['Sector'].dropna().unique()
            if len(sectores_unicos) == 1 and sectores_unicos[0] != "No asignado":
                sector = sectores_unicos[0].upper()
            else:
                sector = 'TODOS'
            title = f"SECTOR {sector} {month} {year}"

            with pd.ExcelWriter(path, engine='openpyxl') as writer:
                # Hoja 1: Resumen de Retrasos
                # Incluir solo los empleados de los sectores presentes en display_df
                all_employees = set()
                sectores_unicos = self.display_df['Sector'].dropna().unique()
                ...
                for emp in sorted(all_employees):
                    emp_data = self.display_df[self.display_df["Nombre"] == emp]
                    if emp_data.empty:
                        # No data - assume never late
                        resumen_data.append({
                            "Nombre": emp,
                            "Dias_tarde_total": 0,
                            "Total_tiempo_tarde": "00:00:00",
                            "Cobrar Presentismo": "Si 100%",
                            "Aclaraciones": "Nunca llegó tarde"
                        })
                    else:
                        # Has data, check for delays
                        all_positive_delays = emp_data[emp_data["Delay"] >= pd.Timedelta(minutes=1)]
                        # Filtrar retrasos normales (<=3 horas) para contar en tiempo total
                        normal_delays = all_positive_delays[all_positive_delays["Delay"] <= pd.Timedelta(hours=3)]
                        # Retrasos extremos (>3 horas): cuentan como días tarde, pero no en tiempo total
                        extreme_delays = all_positive_delays[all_positive_delays["Delay"] > pd.Timedelta(hours=3)]
                        num_extreme = len(extreme_delays)

                        # Dias_tarde_total incluye normales, extremos y días sin fichar entrada
                        days_missing_entry = emp_data["Delay"].isna().sum()
                        total_late_days = len(normal_delays) + num_extreme + days_missing_entry

                        if normal_delays.empty and num_extreme == 0 and days_missing_entry == 0:
                            # No delays at all
                            row = {
                                "Dias_tarde_total": 0,
                            ...
                            cobrar = "Si 100%"
                            aclaracion = "Nunca llegó tarde"
                        else:
                            # Compute stats for normal delays
                            if normal_delays.empty:
                                ...
                                row = {
                                    "Dias_tarde_total": total_late_days,
                                    "Dias_tarde_menor_igual_15min": (group["Delay"] <= pd.Timedelta(minutes=15)).sum(),
                                    "Dias_tarde_16_60": ((group["Delay"] > pd.Timedelta(minutes=15)) & (group["Delay"] <= pd.Timedelta(minutes=60))).sum(),
                                    "Dias_tarde_mayor_60": (group["Delay"] > pd.Timedelta(minutes=60)).sum(),
                                    "Total_tiempo_tarde": group["Delay"].sum(),
                                }
                        # Calcular Presentismo (%) basado en total_late_days
                        delays_16_60 = row["Dias_tarde_16_60"]
                        delays_over_60 = row["Dias_tarde_mayor_60"]
                        ...
                        if total_late_days == 0:
                            ...
                            if num_delays_over_15 == 0:
                                cobrar = "Si 100%"
                                aclaracion = "Llegó tarde 1 vez, ≤15 min"
                            else:
                                cobrar = "Si 100%"
                                aclaracion = "Llegó tarde 1 vez, >15 min"
                            ...
                            if num_delays_over_15 == 0:
                                cobrar = "Si 100%"
                                aclaracion = "Llegó tarde 2 veces, todas ≤15 min"
                            elif num_delays_over_15 == 1:
                                cobrar = "Si 100%"
                                aclaracion = "Llegó tarde 2 veces, una >15 min y una ≤15 min"
                            else:
                                cobrar = "Si 50%"
                                aclaracion = "Llegó tarde 2 veces, ambas >15 min"
                        else:
                            cobrar = "No"  # Más de 2 tardanzas
                            aclaracion = f"Llegó tarde {total_late_days} veces (>2 tardanzas)"
                            if num_extreme > 0:
                                aclaracion += f". Además, llegó más de 3 horas tarde en {num_extreme} días (no fichó ingreso, contado como llegada tarde pero no en horas totales)."
                            if days_missing_entry > 0:
                                aclaracion += f". Además, faltó fichar entrada en {days_missing_entry} días."
                        row["Cobrar Presentismo"] = cobrar
                        row["Aclaraciones"] = aclaracion
                        # Formatear Total_tiempo_tarde como HH:MM:SS
                        total_td = row["Total_tiempo_tarde"]
                        row["Total_tiempo_tarde"] = f"{int(total_td.total_seconds() // 3600):02d}:{int((total_td.total_seconds() % 3600) // 60):02d}:{int(total_td.total_seconds() % 60):02d}"
                        row["Nombre"] = emp
                        resumen_data.append(row)

                df_resumen = pd.DataFrame(resumen_data)
                # Seleccionar solo las columnas requeridas
                df_resumen = df_resumen[["Nombre", "Dias_tarde_total", "Total_tiempo_tarde", "Cobrar Presentismo", "Aclaraciones"]]
                if not df_resumen.empty:
                    # Calcular totales globales (solo para empleados con datos y retrasos)
                    empleados_con_datos = df_resumen[df_resumen["Dias_tarde_total"] > 0]
                    if not empleados_con_datos.empty:
                        total_dias_tarde = empleados_con_datos["Dias_tarde_total"].sum()
                        # Sumar los tiempos totales tarde (convertir de vuelta a Timedelta para sumar)
                        tiempos_validos = empleados_con_datos[empleados_con_datos["Total_tiempo_tarde"] != "00:00:00"]
                        if not tiempos_validos.empty:
                            total_tiempo_tarde = pd.to_timedelta(tiempos_validos["Total_tiempo_tarde"].apply(lambda x: f"0 days {x}")).sum()
                            total_tiempo_str = f"{int(total_tiempo_tarde.total_seconds() // 3600):02d}:{int((total_tiempo_tarde.total_seconds() % 3600) // 60):02d}:{int(total_tiempo_tarde.total_seconds() % 60):02d}"
                        else:
                            total_tiempo_str = "00:00:00"
                        # Agregar fila TOTAL
                        fila_total = pd.DataFrame([{"Nombre": "TOTAL", "Dias_tarde_total": total_dias_tarde, "Total_tiempo_tarde": total_tiempo_str, "Cobrar Presentismo": "", "Aclaraciones": ""}])
                        df_resumen = pd.concat([df_resumen, fila_total], ignore_index=True)
                # Convertir todo a string para evitar problemas de tipos en Excel
                df_resumen = df_resumen.astype(str)
                # Escribir título y luego datos
                pd.DataFrame([title]).to_excel(writer, sheet_name='Resumen Retrasos', index=False, header=False)
                df_resumen.to_excel(writer, sheet_name='Resumen Retrasos', index=False, startrow=1)
                # Hoja 2: Todos los días tarde con fecha y hora
                df_detalle = self.display_df[["Nombre", "Fecha", "Hora", "Delay"]].copy()
                # Filtrar solo retrasos positivos (días tarde) >= 1 minuto
                df_detalle = df_detalle[df_detalle["Delay"] >= pd.Timedelta(minutes=1)]
                if not df_detalle.empty:
                    # Agregar columna Minutos Tarde
                    df_detalle["Minutos Tarde"] = df_detalle["Delay"].apply(
                        lambda x: int(x.total_seconds() // 60) if pd.notna(x) and x != pd.NaT else 0
                    )
                    # Formatear Delay como HH:MM:SS
                    df_detalle["Delay"] = df_detalle["Delay"].apply(
                        lambda x: "" if pd.isna(x) or x == pd.NaT else f"{int(x.total_seconds() // 3600):02d}:{int((x.total_seconds() % 3600) // 60):02d}:{int(x.total_seconds() % 60):02d}"
                    )
                    df_title_detalle = pd.DataFrame([title])
                    df_headers_detalle = pd.DataFrame([df_detalle.columns.tolist()])
                    df_combined_detalle = pd.concat([df_title_detalle, df_headers_detalle, df_detalle], ignore_index=True)
                    df_combined_detalle.to_excel(writer, sheet_name='Dias Tarde', index=False, header=False)
                else:
                    # Si no hay retrasos, crear hoja con mensaje
                    df_title_detalle = pd.DataFrame([title])
                    df_message_detalle = pd.DataFrame({"Mensaje": ["No hay registros de días tarde"]})
                    df_combined_detalle = pd.concat([df_title_detalle, df_message_detalle], ignore_index=True)
                    df_combined_detalle.to_excel(writer, sheet_name='Dias Tarde', index=False, header=False)
                # Hoja 3: Todos los datos procesados
                df_procesados = self.display_df.copy()
                df_title_procesados = pd.DataFrame([title])
                df_headers_procesados = pd.DataFrame([df_procesados.columns.tolist()])
                df_combined_procesados = pd.concat([df_title_procesados, df_headers_procesados, df_procesados], ignore_index=True)
                df_combined_procesados.to_excel(writer, sheet_name='Datos Procesados', index=False, header=False)
            QMessageBox.information(self, "Guardado", f"Archivo guardado en:\n{path}\nIncluye hojas 'Resumen Retrasos', 'Datos Detallados' y 'Datos Procesados' con título '{title}'.")
        except Exception as e:
            QMessageBox.critical(self, "Error guardado", str(e))
    ...
    def exportar_no_asignados_csv(self):
        ...
        if sin.empty:
            ...
            return
        path, _ = QFileDialog.getSaveFileName(self, "Exportar no-asignados (CSV)", "", "Archivos CSV (*.csv)")
        if not path:
            return
        try:
